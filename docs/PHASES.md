...
## Phase 2: DB 연동 + 상세 페이지 + 랭킹 + 직군/학년별 추천

### 2-1. Supabase 설정
- [ ] Supabase 프로젝트 연결 가이드 문서화
...
- [ ] **메인 페이지에 직군/학년 퀵 네비게이션 추가**
  - "당신의 직업은?" 직군 아이콘 그리드
  - "학생이신가요?" 학년 선택 섹션
- [ ] **메인 페이지에 종합 랭킹 TOP 5 위젯**
...
### 2-8. 랭킹 페이지
- [ ] `app/rankings/page.tsx` - 종합 랭킹
  - TOP 100 테이블 (순위, 서비스명, 카테고리, 평점, 주간 변동 ↑↓)
  - 1~3위 강조 디자인 (금/은/동)
  - 카테고리 탭 필터
- [ ] `app/rankings/[category]/page.tsx` - 카테고리별 랭킹
  - 해당 카테고리 TOP 20

### 2-9. 직군별 추천 페이지
- [ ] `app/jobs/page.tsx` - 직군 목록
  - 아이콘 + 이름 + 간단 설명 카드 그리드
  - "당신의 직업을 선택하세요"
- [ ] `app/jobs/[slug]/page.tsx` - 직군 상세
  - 직군 소개 헤더
...
### 2-10. 학년별 추천 페이지
- [ ] `app/education/page.tsx` - 교육 메인
  - 학년별 카드: 초등 저/고, 중, 고, 대학, 교사
  - 각 카드에 대표 추천 AI 미리보기
  - 안전 등급 범례 설명
- [ ] `app/education/[level]/page.tsx` - 학년 상세
  - 학년 소개 (나이대, 설명)
  - **안전 등급별 AI 추천**
    - 🟢 Safe: 자유롭게 사용 가능
...
- [ ] 위자드 로직: 조건에 맞는 서비스를 ranking_score로 정렬
...
| Phase 2 | [ ] 미진행 | DB 연동, 상세 페이지, 랭킹, 직군/학년 |
| Phase 3 | [ ] 미진행 | 검색, 필터, 추천 위자드 |
| Phase 4 | [ ] 미진행 | 인증, 리뷰, 댓글, 북마크, 업보트 |
